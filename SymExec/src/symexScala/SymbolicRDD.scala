...
class SymbolicRDD( /*ofType: VType, */ name: String, es: Array[SymVar]) {
  ...
  def aggregateOnRDD(effect: NonTerminal): NonTerminal = {
    //SymVar, Expr -> Expr
    val l =
      if (effect.left.isInstanceOf[SymVar])
        effect.left.applyEffect(effect.left.asInstanceOf[SymVar], elements(0))
      else null

    val r =
      if (effect.right.isInstanceOf[SymVar])
        effect.right.applyEffect(effect.right.asInstanceOf[SymVar], elements(1))
      else null
