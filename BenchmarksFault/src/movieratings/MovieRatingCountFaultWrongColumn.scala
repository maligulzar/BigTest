...
/**
  ...
  */
object MovieRatingCountFaultWrongColumn extends SparkRDDGenerator{
      ...
      try {
        val averageRating = sc
          ...
          .map { line =>
            val arr = line.split(":")
            val movie_str = arr(0)
            val ratings = arr(1).split(",")(1).split("_")(1) // injecting fault by accessing the wrong column ==> Should lead to crash
