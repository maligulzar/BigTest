...
/**
  ...
  */
object MovieRatingCountFaultWrongOffsets extends SparkRDDGenerator{
      ...
      try {
        val averageRating = sc
          ...
          .map { line =>
            ...
            (movie_str, ratings.substring(0, 3))  // injecting fault by using wrong offsets ==> should lead to crash
          }
          .map { c =>
            val str = c._1
