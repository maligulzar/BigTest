...
/**
  ...
  */
object CommuteTypeFaultWrongPredicate extends SparkRDDGenerator{
    ...
    for(i <- 0 to data1.length-1){
      try{
        ...
        val locations = sc.parallelize(Array(data2(i)))
          ...
          .filter(s => s._2.equals("Palms!")) //  Injecting fault by using the wrong predicate  ==> should produce wrong output
