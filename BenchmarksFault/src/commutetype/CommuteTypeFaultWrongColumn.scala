...
/**
  ...
  */
object CommuteTypeFaultWrongColumn extends SparkRDDGenerator {
    ...
    for(i <- 0 to data1.length-1){
      try{
        val trips = sc.parallelize(Array(data1(i)))
          .map { s =>
            val cols = s.split(",")
            (cols(0), Integer.parseInt(cols(3)) / Integer.parseInt(cols(4)))  // Injecting fault by accesing the wrong column ==> should crash
          }
  ...
  override def execute(input1: RDD[String], input2: RDD[String]): RDD[String] = {
    val trips = input1
      .map { s =>
        val cols = s.split(",")
        (cols(1), Integer.parseInt(cols(2)) / Integer.parseInt(cols(4)))  // Injecting fault by accesing the wrong column ==> should crash
      }
