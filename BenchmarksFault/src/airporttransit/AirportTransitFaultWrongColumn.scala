...
/**
  ...
  */
object AirportTransitFaultWrongColumn extends SparkRDDGenerator {
    ...
    for (i <- 0 to data1.length - 1) {
      try {
        ...
        val map1 = sc.parallelize(Array(data1(i))).map { s =>
          ...
          val diff = getDiff(tokens(2), tokens(4))  // Injecting fault by accessing wrong column ==> Should Crash
