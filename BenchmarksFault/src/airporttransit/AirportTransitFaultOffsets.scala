...
/**
  ...
  */
object AirportTransitFaultOffsets extends SparkRDDGenerator{
      ...
      try {
        ...
        val map1 = sc.parallelize(Array(data1(i))).map { s =>
          def getDiff(arr: String, dep: String): Int = {
            val a_min = Integer.parseInt(arr.substring(2, 4))    // Injecting faults by changing the offsets ==> Should crash
            val a_hr = Integer.parseInt(arr.substring(0, 2))
            val d_min = Integer.parseInt(dep.substring(2, 4))    // Injecting faults by changing the offsets == Should Crash
