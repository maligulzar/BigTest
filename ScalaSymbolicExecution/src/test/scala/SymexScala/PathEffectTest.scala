...
class PathEffectTest extends FlatSpec with BeforeAndAfterAll with Matchers {
    ...
    def fixture =
        new {
            ...
            val pathEffectObj = pe01.conjunctPathEffect(pe23, link)
        }
    ...
    "Test conjuncting PathEffect" should "return conjuncting two path and effects" in {
        val result = fixture.pathEffectObj
        println(result)
        ...
        assert(result.pathConstraint.clauses(0).equals(fixture.c2(0)))
        assert(result.pathConstraint.clauses(1).equals(fixture.c0(0)))
    }

    "Test replace method" should " replaces only the specified variable" in {
        //path: x0 > 100 -> effect : x1 = x0 + 1
        val x0 = new SymVar(Numeric(_Int), "x0")
        val c0 :Array[Clause] = Array(new Clause(x0,
                                                GreaterThan,
        ...
        val pathEffectObj = pe01.conjunctPathEffect(pe23, link)
        val result = pathEffectObj.replace(x0, new SymVar(Numeric(_Int), "R"))
        println(result)
    }
